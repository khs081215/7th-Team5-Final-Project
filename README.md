...
**설계 판단**  
필요한 상위 인원만 뽑으면 되므로 전체 정렬 대신 min-heap top-K 추출(Heapify + K회 Pop)을 선택. 이전 프레임 Winners와 diff를 계산해 상태가 바뀐 적에게만 공격 허가/취소 델리게이트를 실행, 매 주기 전체 재브로드캐스트를 제거.
...
**문제**  
콜백 값 비교 기반 구독 해제, GC 시 소멸된 구독자로 인한 브로드캐스트 크래시 등 구조적 문제 존재.

**해결**  
핸들 기반 구독 관리(`FGameplayMessageHandle`, ID로 O(1) 식별) 도입. `RemoveAtSwap`으로 재정렬 비용 제거, GC 완료 콜백으로 소멸된 리스너 정리해 크래시 차단, 브로드캐스트 중 스냅샷 순회로 재진입 방지.

**설계 판단**  
기존 구조는 구독 해제를 구독자가 직접 호출해야 했고, 해제를 누락한 채 객체가 소멸하면 브로드캐스트 시점에 무효 리스너를 건드려 크래시가 났다. 해제 책임이 구독자 쪽에 남아 있는 한 같은 사고가 반복된다고 보고, 구독 시점에 발급한 핸들로 리스너를 식별하고 GC 완료 콜백에서 소멸된 리스너를 시스템이 스스로 정리하도록 전환. 리스너 배열은 순서 보장이 필요 없어 `RemoveAtSwap`으로 재정렬 비용을 없앴고, 브로드캐스트 도중 구독 상태가 바뀔 수 있어 배열을 직접 순회하는 대신 스냅샷을 떠서 순회하도록 함. 부가로 static wrapper를 두어 채널 호출부를 평균 3줄 → 1줄로 줄이고, 콘솔 명령 기반 디버그 로깅(`ECVF_Cheat`)을 추가.

**결과**  
구독 해제 크래시 제거. 팀 내 전파 후 UI 팀에서 인터페이스로 추상화해 채택, 팀 전체가 GMRouter를 통해 결합 없이 이벤트 통신하는 구조로 정착.
...
**결과**  
게임 스레드 기록 경로에 IO 호출이 없어, 전투 중 파일 IO로 인한 프레임 영향이 구조적으로 발생하지 않음. 적 밸런싱을 담당하면서 적·스킬별 피해량과 피격 빈도를 SQL로 정렬해, 어떤 공격이 실제로 위협적인지를 근거로 수치를 조정. 앞서 도입한 공격 토큰 시스템도 일정 시간당 피격 횟수를 비교해 난이도 개선 효과를 확인하고 토큰 수를 다듬는 데 활용. 스키마는 가드로 막힌 타격을 제외하고 HP가 실제 감소한 경우만 기록해, 실피해를 준 공격을 추적하는 데 초점을 둠.
